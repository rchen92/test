...
--查询使用说明：
--查询语句1到14 使用query 用户登陆后线库(sgehis)，进行查询
--查询语句15到16 是国际板相关查询，使用query用户登国际板库(sgeint)，进行查询
...
--13.当日定价报单
select count(*) "定价当日报单" from fpuser.t_fp_entr_flow;

select count(*) "dj当日报单.offline" 
  from fpuser.t_fp_his_entr_flow t 
 where f_exch_date=(select to_char(sysdate-5, 'yyyymmdd') from dual);
 
--14.当日空价成交单
select count(*) "定价当日成交" from fpuser.t_fp_match_flow;

select count(*) "dj当日成交.offline" 
